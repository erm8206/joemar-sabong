...
@Component({
  ...
  styleUrl: './cashout.component.scss'
})
export class CashoutComponent implements OnInit {
  ...
  async getTransaction(page: number = 1): Promise<void> {
    this.isLoading = true;
    try {
      const res: any = await this._api.get('admin', `/summary-cashout?pageNumber=${page}&pageSize=${this.pageSize}&from=${this.from}&to=${this.to}`);
