...
@Component({
  selector: 'app-player-bet-pick2-history',
  templateUrl: './player-bet-pick2-history.component.html',
  styleUrl: './player-bet-pick2-history.component.scss'
})
export class PlayerBetPick2HistoryComponent implements OnInit {
  playerId: string = '';
  playerName: string = '';
  gameType: string = '';
  ...
  async getBetSummary(page: number = 1): Promise<void> {
    ...
    try {
      const res: any = await this._api.get(
        'user',
        `/lotto-bet-history/pick2/${this.playerId}?pageNumber=${page}&pageSize=${this.pageSize}&search=${encodeURIComponent(this.search)}`
      );
