...
@Component({
  selector: 'app-player-bet-pick3-history',
  templateUrl: './player-bet-pick3-history.component.html',
  styleUrl: './player-bet-pick3-history.component.scss'
})
export class PlayerBetPick3HistoryComponent implements OnInit {
  playerId: string = '';
  playerName: string = '';
  gameType: string = '';
  ...
  async getBetSummary(page: number = 1): Promise<void> {
    ...
    try {
      const res: any = await this._api.get(
        'user',
        `/lotto-bet-history/pick3/${this.playerId}?pageNumber=${page}&pageSize=${this.pageSize}&search=${encodeURIComponent(this.search)}`
      );
