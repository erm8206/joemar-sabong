...
@Component({
  selector: 'app-player-bet-suertres-history',
  templateUrl: './player-bet-suertres-history.component.html',
  styleUrl: './player-bet-suertres-history.component.scss'
})
export class PlayerBetSuertresHistoryComponent implements OnInit {
  playerId: string = '';
  playerName: string = '';
  gameType: string = '';
  ...
  async getBetSummary(page: number = 1): Promise<void> {
    ...
    try {
      const res: any = await this._api.get(
        'user',
        `/lotto-bet-history/suertres/${this.playerId}?pageNumber=${page}&pageSize=${this.pageSize}&search=${encodeURIComponent(this.search)}`
      );
