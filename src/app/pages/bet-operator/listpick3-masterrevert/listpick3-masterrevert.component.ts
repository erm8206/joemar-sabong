...
@Component({
  selector: 'app-listpick3-masterrevert',
  templateUrl: './listpick3-masterrevert.component.html',
  styleUrl: './listpick3-masterrevert.component.scss'
})
export class Listpick3MasterrevertComponent implements OnInit {
  ...
  async getDrawEvents(page: number = 1): Promise<void> {
    this.isLoading = true;
    try {
      const res: any = await this._api.get('betops', `/lotto/master-revert/pick3?pageNumber=${page}&pageSize=${this.pageSize}`);
