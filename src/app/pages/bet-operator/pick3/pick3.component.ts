...
export class Pick3Component implements OnInit {
  model: any = {};
  ...
  async getLottoEvents(page: number = this.pageNumber): Promise<void> {
    this.isLoading = true;
    try {
      const query = `/events-lotto/pick3?pageNumber=${page}&pageSize=${this.pageSize}&startDate=${this.from}&endDate=${this.to}&search=${encodeURIComponent(this.searchTerm)}`;
